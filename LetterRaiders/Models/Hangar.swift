...
enum Hangar {
    ...
    // saves from the earlier capped-bank model — the semantics changed
    // (single persistent pool, no cap) but existing values carry over.
    ...
    // MARK: - Power-up stock (persistent, uncapped)
    //
    ...
    //
    // No cap — players keep what they earn. Fresh installs default to
    ...
    static var zapStock: Int {
        get {
            ...
            return UserDefaults.standard.integer(forKey: bonusZapKey)
        }
        set { UserDefaults.standard.set(max(0, newValue), forKey: bonusZapKey) }
    }
    ...
    static var wildStock: Int {
        get {
            ...
            return UserDefaults.standard.integer(forKey: bonusWildKey)
        }
        set { UserDefaults.standard.set(max(0, newValue), forKey: bonusWildKey) }
    }

    /// Spend `chargePrice` coins for +1 ZAP in the persistent stock.
    /// Returns false (changes nothing) when the player can't afford it.
    @discardableResult
    static func buyZap() -> Bool {
        guard coins >= chargePrice else { return false }
        coins -= chargePrice
        zapStock += 1
        return true
    }
    ...
    /// WILD bonus). Just bumps the persistent stock — no cap, no drain.
